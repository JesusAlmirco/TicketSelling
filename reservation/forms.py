...
class ReservationForm(forms.Form):
    ...
    chosen_seats = forms.CharField(
        label='Chosen seats', 
        max_length=100, 
        widget=forms.Textarea(attrs={'rows': 2, 'readonly': True,'style': 'width:100%'})
    )
...
class ReservationCheckForm(forms.Form):
    ...
    def check_both(reservation_id, email):
        if not Client.objects.filter(reservations__pk=reservation_id, email=email).exists():
            raise ValidationError('Reservation for {} address with {} number does not exist'.format(reservation_id, email))
